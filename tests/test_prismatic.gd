...
func run() -> void:
    ...
    check(is_equal_approx(Layer.star_radius(4.0),1.0) and is_equal_approx(Layer.star_radius(4.5),1.9), "White light grows during first 500ms of full hold")
    check(is_equal_approx(Layer.star_radius(8.0),1.9) and is_zero_approx(Layer.star_radius(8.5)), "White light shrinks to zero over first 500ms of outbreath")
    check(is_zero_approx(Layer.star_radius(15.99)) and is_zero_approx(Layer.star_radius(0)), "Light remains absent through empty pause and wraps continuously")
