...
func run() -> void:
    ...
    check(ExperienceMath.inhale_gate(2.5) > 0.99, "Inhale gently reaches full intensity")
    ...
    for length: float in [12.0, 14.0]:
        close(ExperienceMath.inhale_visibility(length - 0.5, length), 0, "Lead-in begins at zero")
        check(ExperienceMath.inhale_visibility(length - 0.25, length) > 0, "Incoming particles gather before inhale cue")
        ...
        check(ExperienceMath.inhale_front(length - 0.25, length) < 0.25, "Early lead-in stays near the orb")
        close(ExperienceMath.inhale_front(1.4, length), 1.14, "Full route visible early in inhale")
    ...
    check((exhale_material.get_shader_parameter("exhale_origin") as Vector3).is_equal_approx(director.breath_center.global_position), "Exhale starts at the shared neck center")
    check((exhale_material.get_shader_parameter("breath_basis") as Basis).is_equal_approx(director.breath_center.global_basis), "Exhale follows tracked orientation")
    ...
    check((director.inhale.process_material.get_shader_parameter("inhale_target") as Vector3).is_equal_approx(director.breath_center.global_position), "Inhale converges to the shared neck center")
    ...
    arc.seek(13.9, true)
    director.elapsed = 13.9
    director.clock.update_from_position(13.9, 13.9)
    director._push_visuals(0.0)
    check(float(director.inhale.process_material.get_shader_parameter("inhale_visibility")) > 0.0, "First visible inhale also has a lead-in")
    arc.seek(475, true)
