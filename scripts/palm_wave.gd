...
func sample(point: Vector3, facing: float, valid: bool, delta: float) -> bool:
    var in_region: bool = valid and point.is_finite() and facing > 0.60 and point.z < -0.20 and point.z > -0.85 and absf(point.x) < 0.65 and absf(point.y) < 0.42
    if not in_region:
        absent += delta
        reset()
        if absent >= 0.45:
    ...
    if elapsed >= 0.20 and samples >= 5 and absf(lateral) >= 0.20:
        disarm()
        return true
    return false
