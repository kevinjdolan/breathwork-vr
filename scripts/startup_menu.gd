...
func _process(delta: float) -> void:
    ...
    if absf(local_direction.z) > 0.001:
        ...
        if distance > 0:
            for index: int in range(cards.size()):
                ...
                if absf(offset.x) < 0.54 and absf(offset.y) < 0.145:
                    hovered = index
    if hovered != selected:
        selected = hovered
        dwell = 0.0
    if selected >= 0:
        ...
        dwell = 0.0
        status.text = "Look at a world for 2.5 seconds to begin" if get_viewport().use_xr else "Click a world or press 1–9 to begin"
    for index: int in range(cards.size()):
...
func _unhandled_input(event: InputEvent) -> void:
    ...
    if event is InputEventKey and event.pressed and not event.echo:
        ...
        if key >= KEY_1 and key <= KEY_9 and key - KEY_1 < entries.size():
            _choose(key - KEY_1)
        elif key == KEY_R:
            _anchor()
