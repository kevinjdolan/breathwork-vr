...
- Incoming particles should visibly travel from the source toward a tracked mouth target throughout the inhale. Avoid increasing their speed to meet a phase deadline. A gentle pre-cue reveal and a long, smooth terminal taper can avoid sudden onset/cutoff. Keep points visible near the mouth while fading before they cross the eyes.
- Use headset-relative up/right for paths and world-space mouth positions. Separate outgoing color, paths, lifetime, and visibility so outgoing remnants do not obscure the next inhale. Preserve accepted timing and PCM samples when only the other phase is changing.
